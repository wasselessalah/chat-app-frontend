...
export default function SignInPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // setIsGoogleLoading(false); // Usually redirects
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4 dark:bg-zinc-950 font-sans">
      <div className="w-full max-w-md space-y-8 rounded-xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <Button variant="outline" type="button" onClick={handleGoogleSignIn} disabled={loading || isGoogleLoading}>
          {isGoogleLoading ? (
            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
          ) : (
        ...
        <p className="text-center text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <Link href="/sign-up" className="font-medium text-zinc-900 hover:underline dark:text-zinc-50">
            Sign up
          </Link>
        </p>
      </div>
